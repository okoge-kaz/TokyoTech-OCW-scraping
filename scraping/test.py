...
replace_inappropriate_string = lambda s: str(s).replace('\t', ' ').replace('\n', ' ').replace('\xa0', ' ').replace('\u3000', ' ').strip(' ')
...
def get_department_list( year=2020, lang="JA"):
    ...
    left_body_section_li_tags = [elem for elem in left_body_section.ul if elem.name == "li" ]
    ...
    def extract_department_name_from_li_tag(li_tag_element, department_name_map):
        ...
        if li_tag_element.a == None:
            return False
        # department_name: 学院名
        department_name = str(replace_inappropriate_string(li_tag_element.a.string))

        major_name_and_url_map = {}
        for li_element in li_tag_element.ul:
            if li_element.name == 'li':
                extract_major_name_from_li_tag(li_element, major_name_and_url_map)
        
        department_name_map[department_name] = major_name_and_url_map # map: (key: department_name::string , value: major_name_and_url_map::dictionary[key::string, url::string ])
        return True
    ...
    def extract_major_name_from_li_tag(li_tag_element, major_name_map):
        ...
        full_url = url_munipulate(li_tag_element.a['href']) if li_tag_element.a['href'] != '#' else url
...
def get_major_all_course_list(major_name, major_url, retry_limit=5):
    ...
    for _ in range(retry_limit):
        ...
        for table in tables:
            ...
            for tr in trs:
                shaping_tables.append(get_course_detail_informaion_from_tr_tag(major_name, tr))
    ...
    for td_tag_element in td_tag_elements:
        if td_tag_element.name == 'td' and (td_tag_element.get('class') is not None):
            if str(td_tag_element).find('width_code')!=-1 and td_tag_element.string is not None:
                course_information['courseId'] = replace_inappropriate_string(str(td_tag_element.string))
                ...
                # print()
                course_information['courseDigit'] = (int)( (replace_inappropriate_string(str(td_tag_element.string)))[5] )
                ...
                for element in td_tag_element:
                    if element.name == 'a':
                        teachers.append(replace_inappropriate_string(str(element.string)))
